...
项目通过 `.python-version` 固定使用 Python 3.13，并通过 `uv.lock` 固定环境。进入项目目录后初始化环境：
...
该文件已加入 `.gitignore`。如需将配置放到其他位置，可使用 `--config /path/to/config.json`。
...
```bash
uv run python rc_image.py generate --prompt "一只戴着太空头盔的橘猫，电影级光影"
```

指定尺寸、分辨率和数量：

```bash
uv run python rc_image.py generate \
  --prompt "未来城市雨夜海报" \
  --size 16:9 \
  --image-size 2K \
  --n 2
...
工具默认使用模型 `gpt-image-2`，向生成接口发送 `async: true`，取得 `task_id` 后持续轮询。图片默认保存到：
...
```bash
uv run python rc_image.py fetch task_0123456789abcdef0123456789abcdef
```
...
```bash
uv run python rc_image.py --help
uv run python rc_image.py generate --help
uv run python rc_image.py fetch --help
```
